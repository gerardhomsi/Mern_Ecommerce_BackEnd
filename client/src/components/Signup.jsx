      ...
      {inputFields.map(({ name, value, type, placeholder, icon, d }) => (
        <div className="input-group mb-3" key={name}>
          <span className="input-group-text" id="basic-addon1">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className={`bi bi-${icon}`} viewBox="0 0 16 16">
              <path d={d} />
            </svg>
          </span>
          <input name={name} value={value} type={type} className="form-control" placeholder={placeholder} onChange={handleChange} />
        </div>
      ))}
